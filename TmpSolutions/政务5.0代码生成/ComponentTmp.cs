...
^##^    /// 表[#TableName#]存取
^##^    /// 关键字段为[#PrimaryKey#]
^##^    public class #TableName#:GS.IGSAccess<#TableName#Models>
^##^    {
";
...
^##^            {
^##^                ht.Add(#TableName#Models.Column.#ColumnName#, result.#ColumnName#);
^##^            }
";
            for (int i = 0; i < listColumnNames.Count; i++)
            {
...
^##^        {
^##^            if (string.IsNullOrWhiteSpace(result.#ColumnName#.ToString()))
^##^            {
^##^                throw new Exception(""关键字的值不能为空"");
^##^            }
^##^                
...
";
            ...
            tmpRes = @"
^##^            return iDb.SaveData(""#TableName#"", ht, ""#PrimaryKey#"");
^##^        }
...
^##^        {
^##^            string strSql = ""select count(*) from #TableName# where 1=1 "" + sFilter;
