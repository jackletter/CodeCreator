...
namespace CodeCreator_JACKOA_Frame
{
...
^##^        /// <returns></returns>
^##^        public static #TableName# Get(IDbAccess iDb, string sFilter)
^##^        {
...
^##^            {";
            ...
            res += tmpRes.Replace("^##^", "");
            tmpRes = @"
^##^                    result.#ColumnName# = ds.Tables[0].Rows[0][#TableName#.Column.#ColumnName#].ToString();";
            for (int i = 0; i < listColumnNames.Count; i++)
            {
...
^##^        public static bool Add(IDbAccess iDb, #TableName# result)
^##^        {
...
^##^        /// <param name=""result"">表对象</param>
^##^        /// <returns>保存是否成功</returns>
^##^        public static bool Save(IDbAccess iDb, #TableName# result)
^##^        {
^##^            if (result==null||string.IsNullOrWhiteSpace(result.#PrimaryKey#))
^##^            {
^##^                throw new Exception(""模型或模型的关键字不能为空"");
...
^##^        public static int Delete(IDbAccess iDb, string sFilter)
^##^        {
^##^            if (string.IsNullOrWhiteSpace(sFilter))
^##^            {
^##^                throw new Exception(""条件字符串不能为空"");
...
^##^        public static int GetDataCount(IDbAccess iDb,string sFilter)
^##^        {
...
^##^        public static List<#TableName#> SearchList(IDbAccess iDb,string sFilter,string sOrderColumn,string sOrderType,string sOrderStr)
^##^        {
...
^##^        public static SearchResult<#TableName#> SearchListByPage(IDbAccess iDb, int iPageSize, int iPageIndex, string sFilter, string sOrderColumn, string sOrderType,string sOrderStr)
^##^        {
^##^            SearchResult<#TableName#> sr = new SearchResult<#TableName#>();
...
";
            ...
            res += tmpRes;
            tmpRes = @"
^##^                result.#ColumnName# = ds.Tables[0].Rows[ii][#TableName#.Column.#ColumnName#].ToString();
";
            for (int i = 0; i < listColumnNames.Count; i++)
            {
                if (listColumnTypes[i].ToUpper() == "datetime".ToUpper())
...
^##^        public static DataSet Search(IDbAccess iDb,string sFilter,string sOrderColumn,string sOrderType,string sOrderStr)
^##^        {
^##^            string sOrderString = """";
^##^
^##^			if (!string.IsNullOrWhiteSpace(sOrderStr))
